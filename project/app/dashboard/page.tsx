    ...
    <div className="min-h-screen pt-24 pb-20">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-white/60">Here is your dashboard overview.</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <DataCard 
            ...
            renderItem={item => <li key={item.id} className="flex justify-between items-center p-2 rounded-lg hover:bg-white/5"><span>{item.project_name}</span><span className="text-cosmic-accent">{formatPrice(item.total_amount)}</span></li>}
          />
          <DataCard 
            ...
            renderItem={item => <li key={item.id} className="flex justify-between items-center p-2 rounded-lg hover:bg-white/5"><span>{item.website_type}</span><span className="text-white/70">{item.status}</span></li>}
          />
          <DataCard 
            title="Notifications" 
            data={notifications} 
            ...
            renderItem={item => <li key={item.id} className="p-2 rounded-lg hover:bg-white/5">{item.message}</li>}
          />
          <DataCard 
            title="Wishlist" 
            data={wishlists} 
            ...
            renderItem={item => <li key={item.id} className="p-2 rounded-lg hover:bg-white/5">{item.name}</li>}
          />
        </div>
      </div>
    </div>

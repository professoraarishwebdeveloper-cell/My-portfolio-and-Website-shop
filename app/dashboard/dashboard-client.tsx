...
const StatCard = ({ label, value, isLoading }: { label: string; value: number; isLoading: boolean }) => (
  <div className="glass-card p-6">
    ...
    {isLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : <p className="text-2xl font-bold text-white">{value}</p>}
  </div>
)
...
export function DashboardClient() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [profileData, quotationsData, ordersData, notificationsData, invoicesData, wishlistsData] = await Promise.all([
        ...
        if (profileData.error) {
          logDevelopmentError('dashboard-profile', profileData.error)
        }

        setProfile(
          profileData.data ?? authProfile ?? {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          <DataCard
            ...
            renderItem={(item) => (
              <li key={item.id} className="flex items-center justify-between rounded-lg p-2 hover:bg-[#0d1528]">
                <span className="text-slate-100">{item.project_details?.websiteName ?? item.website_type}</span>
          ...
          <DataCard
            ...
            renderItem={(item) => (
              <li key={item.id} className="flex items-center justify-between rounded-lg p-2 hover:bg-[#0d1528]">
                <span className="text-slate-100">{item.website_type}</span>
